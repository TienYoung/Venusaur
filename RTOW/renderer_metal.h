...
public:
    RendererMetal(std::shared_ptr<venusaur::OutputBuffer> outputBuffer, const std::vector<char>& optixIR)
        : venusaur::RendererBase(outputBuffer, 1) {
        {
            ...
            CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_centerBuffer),
                                  sphereCenter.data(),
                                  sizeof(float3) * sphereCenter.size(),
                                  cudaMemcpyHostToDevice));

            CUdeviceptr d_radiusBuffer;
            CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_radiusBuffer), sizeof(float) * sphereRadius.size()));
            CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_radiusBuffer),
                                  sphereRadius.data(),
            ...
            CUDA_CHECK(cudaMemcpy(
                reinterpret_cast<void*>(d_sbtIndexBuffer), indices.data(), sizeof(indices), cudaMemcpyHostToDevice));
        ...
        OPTIX_CHECK_LOG(optixBuiltinISModuleGet(
            m_context, &moduleCompileOptions, &pipelineCompileOptions, &sphereISOptions, &sphereModuleIS));
