...
namespace RayTracingInOneWeekend
{
	...
	class RendererSphere : public Venusaur::RendererBase
    {
        public:
		RendererSphere(std::shared_ptr<Venusaur::OutputBuffer> outputBuffer, const std::vector<char>& optixIR) :
		...
		{
			{
				...
				auto sphereCenter = make_float3( 0.f, 0.f, -1.f );
				auto sphereRadius = 0.5f;

				CUdeviceptr d_centerBuffer;
				CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_centerBuffer), sizeof(float3)));
				CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_centerBuffer), &sphereCenter, sizeof(float3), cudaMemcpyHostToDevice));

				CUdeviceptr d_radiusBuffer;
				CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_radiusBuffer), sizeof(float)));
				CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_radiusBuffer), &sphereRadius, sizeof(float), cudaMemcpyHostToDevice));
